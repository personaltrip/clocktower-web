<template>
  ...
  >
    ...
    >
      <h3>
        <span v-if="session.isSpectator && isDemon">恶魔的伪装</span>
        <span v-else-if="session.isSpectator">不在场的角色</span>
        <span v-else>恶魔的伪装</span>
      ...
      <ul>
        ...
        >
          <Token :role="currentBluffs[index - 1] || {}"></Token>
        </li>
      </ul>
    </div>
...
export default {
  ...
  computed: {
    ...
    isDemon() {
      const myPlayer = this.players.find(p => p.id === this.session.playerId);
      return myPlayer && myPlayer.role && myPlayer.role.team === "demon";
    },
    showBluffs() {
      if (!this.players.length) return false;
      // 说书人始终显示；观众只要加入了房间就显示（空 token 或 ST 伪装）
      return !this.session.isSpectator || !!this.session.sessionId;
    },
    currentBluffs() {
      if (this.isDemon) {
        const myPlayer = this.players.find(p => p.id === this.session.playerId);
        return (myPlayer && myPlayer.bluffs) || [];
      }
      // 说书人看到自己设置的伪装面板，观众看到空
      return this.session.isSpectator ? [] : this.bluffs;
    }
  },
  data() {
    return {
  ...
  methods: {
    ...
    removePlayer(playerIndex) {
      ...
      if (
        confirm(
          `确定要移除玩家 ${this.players[playerIndex].name} 吗？`
        )
      ) {
