<template>
  ...
  >
    ...
    <div class="warning" v-if="hasSelectedSetupRoles">
      ...
      <span>
        警告⚠️: 目前选择的角色会修改游戏的初始角色配置！随机分发器不会识别这些角色的功能。建议说书人手动调整要分发的角色。
      </span>
    </div>
...
export default {
  ...
  methods: {
    selectRandomRoles() {
      // Build selection in a plain object first, then assign to roleSelection
      // all at once. This prevents a Vue 2 reactivity bug: if we reset
      // roleSelection = {} first, the selectedRoles computed evaluates against
      // the empty object and gets cached at 0. Subsequent $set calls add team
      // keys, but selectedRoles has no dependency on those new keys and never
      // re-evaluates — leaving the assign button stuck at "0 个角色".
      const byTeam = {};
  ...
  watch: {
    ...
    allowMultiple(checked) {
      if (!checked) {
        // When disabling multiple selection, cap all selected values to 1
        Object.values(this.roleSelection).forEach(roles => {
          roles.forEach(role => {
            if (role.selected > 1) role.selected = 1;
          });
        });
      }
    }
